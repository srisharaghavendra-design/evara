  ...
  return (
    <div style={{ animation: "fadeUp .2s ease" }}>
      ...
      {activeEvent && showMorningBrief && (() => {
        ...
        // Smart insight — use campaigns from props if available (more up to date)
        const activeCampaigns = propCampaigns || campaigns;
        const approvedCount = activeCampaigns.filter(c => c.status === "approved" || c.status === "sent" || c.status === "scheduled").length;
        const draftCount = activeCampaigns.filter(c => c.status === "draft").length;
        let insight = null;
        ...
        else if (sentCount === 0 && approvedCount > 0 && draftCount === 0 && !lpPublished) insight = { msg: `All ${approvedCount} email${approvedCount>1?"s":""} approved ✓ — now set up your landing page.`, cta: "Go to Step 2 →", action: () => setView("landing"), color: C.green };
        else if (sentCount === 0 && approvedCount > 0 && draftCount === 0 && lpPublished) insight = { msg: `Steps 1 & 2 complete ✓ — schedule your emails to launch the campaign.`, cta: "Go to Step 3 →", action: () => setView("schedule"), color: C.green };
